...
export function AdminTabsNav({ role, badgeCounts = {} }: { role: AppRole; badgeCounts?: AdminTabBadgeCounts }) {
  ...
  return (
    <ScrollableRow className="flex items-center gap-1 p-1 bg-secondary/60 rounded-xl border border-border/40 select-none">
      {visibleTabs.map((tab) => {
        // Cas particulier de "/admin" (Vue d'ensemble) : c'est un préfixe de
        // TOUTES les autres pages d'administration ("/admin/users" etc.) —
        // sans cette exception, l'onglet resterait actif partout ailleurs
        // dans l'espace équipe, en plus de l'onglet réellement courant.
        const isActive = tab.href === "/admin" ? pathname === "/admin" : pathname === tab.href || pathname.startsWith(`${tab.href}/`);
