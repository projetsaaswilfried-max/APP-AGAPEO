 ...
 * `/api/webhooks/chariow/weekly`, `/api/webhooks/chariow/half-month`) car
 * Chariow interdit de réutiliser une même URL sur deux Pulses différents ;
 * chaque route appelle cette fonction partagée avec son propre `plan`, ce
 * qui détermine à la fois quel secret vérifier et quelle durée/valeur
 * appliquer. Un Pulse unique partagé par tous les plans a été tenté puis
 * abandonné (paiements reçus mais jamais activés côté Chariow) — retour à un
 * Pulse dédié par produit. Chariow ne gère pas les abonnements récurrents —
 * chaque `successful.sale` correspond à un paiement unique d'accès Premium,
